...
------------
  ...
  python resolve_i18n_tags.py --json scripts/fsdash/i18n/en.json --root src --dry-run

  # Apply replacements in-place:
  python resolve_i18n_tags.py --json scripts/fsdash/i18n/en.json --root src
"""
#!/usr/bin/env python3
...
def apply_transform_pipeline(s: str, basic_mod: Optional[str], chain: str, stats: dict) -> str:
...
def apply_modifier(s: str, mod: Optional[str]):
    if not mod:
        return s
    if mod == 'upper':
        return s.upper()
