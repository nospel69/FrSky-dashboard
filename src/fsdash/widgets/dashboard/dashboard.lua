...
local function getThemeForState(state)
    ...
    elseif value == "RT-RC" or value == "@RT-RC" then
        value = "system/@rt-rc"
    end

    return value or userPrefs["theme_" .. state] or DEFAULT_THEME
end
...
local function loadStateScript(themeFolder, state, isFallback)
    ...
    local scriptPath = basePath .. folder .. "/" .. scriptName
...
local function reloadTheme()
    ...
    loadedStates = {
        ...
        postflight = loadStateScript(getThemeForState("postflight"), "postflight")
    }
...
local function paintObjects()
    local module = loadedStates[currentState]
    if not module then
        dashboard.utils.screenError("Your theme did not load correctly. Falling back to default theme.", true, 0.5)
        return
    end
    ...
    local overlay = getOverlayMessage(currentState)
    if overlay then
        local windowW, windowH = lcd.getWindowSize()
        local offsetY = 0
        ...
        dashboard.overlaymessage(0, offsetY, windowW, windowH - offsetY, overlay)
    end
end
