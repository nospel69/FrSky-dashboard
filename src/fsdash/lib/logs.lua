...
function logs.getSummary()
    local prefs = dashx.session and dashx.session.modelPreferences or nil

    return {
        craftName = (dashx.session and dashx.session.craftName) or (model.name and model.name()) or "Model",
        flightCount = tonumber(dashx.ini.getvalue(prefs, "general", "flightcount")) or 0,
        lastFlightTime = tonumber(dashx.ini.getvalue(prefs, "general", "lastflighttime")) or 0,
        totalFlightTime = tonumber(dashx.ini.getvalue(prefs, "general", "totalflighttime")) or 0
    }
end
