...
local function openPage(pageIdx, title, script)
    ...
    dashx.app.ui.fieldHeader("Settings" .. " / " .. "General")
    ...
    dashx.app.formLines[dashx.session.formLineCnt] = form.addLine("Icon Size")
    dashx.app.formFields[formFieldCount] = form.addChoiceField(dashx.app.formLines[dashx.session.formLineCnt], nil, {{"TEXT", 0}, {"SMALL", 1}, {"LARGE", 2}}, function()
        if dashx.preferences and dashx.preferences.general and dashx.preferences.general.iconsize then
            return settings.iconsize
        else
            return 1
        end
    end, function(newValue) if dashx.preferences and dashx.preferences.general then settings.iconsize = newValue end end)
    ...
    local buttons = {
        {
            ...
            action = function()
                ...
                for key, value in pairs(settings) do dashx.preferences.general[key] = value end
