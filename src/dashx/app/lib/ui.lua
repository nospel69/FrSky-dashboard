    ...
    dashx.app.dialogs.noLink = form.openProgressDialog({
        ...
        wakeup = function()
            ...
            if not utils.ethosVersionAtLeast() then
                msg = string.format("%s < V%d.%d.%d", string.upper("ethos"), table.unpack(dashx.config.ethosVersion))
            elseif not dashx.tasks.active() then
                msg, invalid, abort = "Please enable the background task.", true, false
            elseif not moduleEnabled and not app.offlineMode then
                msg, invalid = "Please check your rf module is turned on.", true
            elseif not (sensorSport or sensorElrs) and not app.offlineMode then
                msg, invalid = "Please check you have discovered all sensors.", true
            end
...
function ui.progressDisplaySave(message)
    ...
    local title = "Saving..."
    if not message then message = "Saving data to flight controller..." end
...
function ui.progressDisplaySaveValue(value, message)
    if value >= 100 then
        if dashx.app.dialogs.save then dashx.app.dialogs.save:value(value) end
...
function ui.enableAllFields() for _, field in ipairs(dashx.app.formFields) do if type(field) == "userdata" then field:enable(true) end end end

function ui.disableAllNavigationFields() for i, v in pairs(dashx.app.formNavigationFields) do if x ~= v then v:enable(false) end end end

function ui.enableAllNavigationFields() for i, v in pairs(dashx.app.formNavigationFields) do if x ~= v then v:enable(true) end end end
...
function ui.progressDisplayIsActive() return dashx.app.dialogs.progressDisplay or dashx.app.dialogs.saveDisplay or dashx.app.dialogs.progressDisplayEsc or dashx.app.dialogs.nolinkDisplay or dashx.app.dialogs.badversionDisplay end
...
function ui.openMainMenu()
    ...
    for i in pairs(dashx.app.gfx_buttons) do if i ~= "mainmenu" then dashx.app.gfx_buttons[i] = nil end end

    if not dashx.utils.ethosVersionAtLeast(config.ethosVersion) then return end
    ...
    for idx, section in ipairs(MainMenu.sections) do
        local hideSection = (section.ethosversion and dashx.session.ethosRunningVersion < section.ethosversion) or (section.mspversion and (dashx.session.apiVersion or 1) < section.mspversion) or (section.developer and not dashx.preferences.developer.devtools)
        ...
        if not hideSection then
            ...
            for pidx, page in ipairs(MainMenu.pages) do
                if page.section == idx then
                    local hideEntry = (page.ethosversion and not dashx.utils.ethosVersionAtLeast(page.ethosversion)) or (page.mspversion and (dashx.session.apiVersion or 1) < page.mspversion) or (page.developer and not dashx.preferences.developer.devtools)
                    ...
                    if not hideEntry then
                        ...
                        if dashx.preferences.menulastselected["mainmenu"] == pidx then dashx.app.formFields[pidx]:focus() end
...
function ui.fieldChoice(i)
    ...
    formFields[i] = form.addChoiceField(formLines[dashx.session.formLineCnt], posField, tbldata, function()
...
function ui.fieldNumber(i)
    ...
    formFields[i] = form.addNumberField(formLines[dashx.session.formLineCnt], posField, minValue, maxValue, function()
...
function ui.fieldText(i)
    ...
    formFields[i] = form.addTextField(formLines[dashx.session.formLineCnt], posField, function()
        if not fields or not fields[i] then
            ui.disableAllFields()
            ui.disableAllNavigationFields()
            ui.enableNavigationField('menu')
...
function ui.openPageRefresh(idx, title, script, extra1, extra2, extra3, extra5, extra6) dashx.app.triggers.isReady = false end
...
function ui.openPage(idx, title, script, extra1, extra2, extra3, extra5, extra6)
    ...
    if dashx.app.Page.headerLine then
        ...
        form.addStaticText(headerLine, {x = 0, y = dashx.app.radio.linePaddingTop, w = dashx.session.lcdWidth, h = dashx.app.radio.navbuttonHeight}, dashx.app.Page.headerLine)
    end
    ...
    if dashx.app.Page.fields then
        for i, field in ipairs(dashx.app.Page.fields) do
            ...
            local valid = (field.apiversion == nil or dashx.utils.round(field.apiversion, 2) <= version) and (field.apiversionlt == nil or dashx.utils.round(field.apiversionlt, 2) > version) and (field.apiversiongt == nil or dashx.utils.round(field.apiversiongt, 2) < version) and
                              (field.apiversionlte == nil or dashx.utils.round(field.apiversionlte, 2) >= version) and (field.apiversiongte == nil or dashx.utils.round(field.apiversiongte, 2) <= version) and (field.enablefunction == nil or field.enablefunction())
...
function ui.openPageDashboard(idx, title, script, source, folder)
    ...
    dashx.app.formNavigationFields['menu'] = form.addButton(line, {x = x, y = dashx.app.radio.linePaddingTop, w = buttonW, h = dashx.app.radio.navbuttonHeight}, {
        ...
        press = function()
            ...
            dashx.app.ui.openPage(pageIdx, "Dashboard", "settings/tools/dashboard_settings.lua")
        end
    })
            ...
            local buttons = {
                {
                    ...
                    action = function()
                        ...
                        if dashx.app.Page.write then dashx.app.Page.write() end
...
function ui.navigationButtons(x, y, w, h)
    ...
    if navButtons.tool ~= nil and navButtons.tool == true then
        dashx.app.formNavigationFields['tool'] = form.addButton(line, {x = toolOffset, y = y, w = wS, h = h}, {text = "*", icon = nil, options = FONT_S, paint = function() end, press = function() dashx.app.Page.onToolMenu() end})
    end
    ...
    if navButtons.help ~= nil and navButtons.help == true then
        ...
        if help then
            ...
            dashx.app.formNavigationFields['help'] = form.addButton(line, {x = helpOffset, y = y, w = wS, h = h}, {text = "?", icon = nil, options = FONT_S, paint = function() end, press = function() end})
...
function ui.openPageHelp(txtData, section)
    ...
    form.openDialog({width = dashx.session.lcdWidth, title = "Help - " .. dashx.app.lastTitle, message = message, buttons = {{label = "CLOSE", action = function() return true end}}, options = TEXT_LEFT})
end
