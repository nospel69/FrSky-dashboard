...
local function buildLogColumns()
    ...
    return {
        {name = "voltage", keyindex = 1, keyname = "Voltage", keyunit = "V", keyminmax = 1, color = colors.voltage, pen = PEN_SOLID, graph = true},
        {name = "current", keyindex = 2, keyname = "Current", keyunit = "A", keyminmax = 1, color = colors.current, pen = PEN_SOLID, graph = true},
        {name = "rpm", keyindex = 3, keyname = "Headspeed", keyunit = "rpm", keyminmax = 1, keyfloor = true, color = colors.rpm, pen = PEN_SOLID, graph = true},
        {name = "temp_esc", keyindex = 4, keyname = "ESC Temp", keyunit = "C", keyminmax = 1, color = colors.temp_esc, pen = PEN_SOLID, graph = true},
        {name = "throttle_percent", keyindex = 5, keyname = "Throttle", keyunit = "%", keyminmax = 1, color = colors.throttle_percent, pen = PEN_SOLID, graph = true}
    }
end
...
local function getIconSize()
    local prefs = dashx.preferences and dashx.preferences.general or nil
...
local function refreshEntries()
    tool.state.entries = dashx.logs.getRecentEntries() or {}
end
...
local function extractHourMinute(filename)
    ...
    return tostring(filename or "Unknown")
end
...
local function formatDate(isoDate)
    ...
    return isoDate or "Unknown Date"
end
...
local function paintView()
    ...
    if tool.state.loadError then
        paintLoadingMessage("Log load failed", tool.state.loadError)
        return
    end
